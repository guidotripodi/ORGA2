...
{
	nodo** nivel = nivelNodo(t,t.->raiz); no se si ya esta en struct q te da el nivel
	Nodo* c = insertar_nodo_en_nivel(nivel, &p);
	while( p.->siguiente /= NULL)
	{
		p = p.->siguiente;
		if(c.->hijos /= NULL)
		{
		nivel = nivelNodo(t, c.->hijos);
		}
		else
		{
		nivel++;
		}
		c = insertar_nodo_en_nivel(nivel, &p);
	}
}
...
{
	bool res = false;
	while(res == true && p.->hijos /= NULL && p.->siguiente /= NULL)
	{
	if(not(nodoDef(t,p)))
	{
		res = false;
